...
def merge_csv_files(directory_path):
    ...
    csv_files = [f for f in os.listdir(directory_path) if f.endswith('readpair_counts.csv')]
    ...
    for csv_file in csv_files:
        ...
        if n==0:
            ...
            merged_df = pd.merge(merged_df, current_df, on='reference_name', how='outer')
        n+=1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Merge all read pair count csv file in the folder")
    parser.add_argument('directory_path', type=str, help='Path to the folder with read pair count csv files')
